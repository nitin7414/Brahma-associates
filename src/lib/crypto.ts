 ...
 * Compact, fast, and does not require native Node crypto or external dependencies.
 */
export function sha256(ascii: string): string {
  ...
  for (let i = 0; i < asciiBitLength; i += 8) {
    words[i >> 5] |= ascii.charCodeAt(i / 8) << (24 - (i % 32));
  }
  ...
  for (let i = 0; i < 8; i++) {
    const word = hash[i];
    for (let k = 0; k < 4; k++) {
      hex += ((word >> (24 - k * 8)) & 0xff).toString(16).padStart(2, '0');
    }
  }
  return hex;
}
